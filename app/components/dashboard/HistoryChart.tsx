...
const HistoryChart: FC<IHistoryChart> = ({ poolingInterval }) => {
  ...
  return (
    <div className="mt-2 h-[50rem]">
        ...
        {granularityList.map((g) => (
          <button
            ...
            className={`px-2 border-2 rounded ${
              granularity === g ? "border-primary" : "black"
            }`}
      ...
      <select
        className="px-2 border-2 rounded bg-black"
        onChange={(e) => setWindow(e.target.value)}
      >
        {validWindows[granularity].map((w) => (
          <option key={w} value={w} selected={w === window}>
            {w}
          </option>
        ))}
      </select>
